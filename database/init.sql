...
CREATE TABLE IF NOT EXISTS todos (
    ...
    category VARCHAR(50) DEFAULT 'WORK' COMMENT '分类：WORK(工作), STUDY(学习), LIFE(生活)',
...
('完成项目需求文档', '编写新功能的需求文档，包括用户故事和验收标准', 'PENDING', 'HIGH', 'WORK', DATE_ADD(NOW(), INTERVAL 2 DAY), TRUE),
('学习Angular基础', '完成Angular官方教程的基础部分', 'IN_PROGRESS', 'MEDIUM', 'STUDY', DATE_ADD(NOW(), INTERVAL 5 DAY), FALSE),
('健身锻炼', '去健身房进行有氧和力量训练', 'PENDING', 'LOW', 'LIFE', DATE_ADD(NOW(), INTERVAL 1 DAY), TRUE),
('周末聚餐安排', '安排周末与朋友的聚餐，预定餐厅', 'COMPLETED', 'MEDIUM', 'LIFE', DATE_SUB(NOW(), INTERVAL 1 DAY), FALSE),
('代码审查', '审查团队成员提交的代码', 'PENDING', 'HIGH', 'WORK', DATE_ADD(NOW(), INTERVAL 3 HOUR), TRUE);
